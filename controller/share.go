...
func SharePass(c *gin.Context) {
	fmt.Println("访问share，调用SharePass，渲染分享文件页面")
	f := c.Query("f")
...
func DownloadShareFile(c *gin.Context) {
	fmt.Println("访问shareDownload，调用DownloadShareFile，下载分享文件")
	fileId := c.Query("id")
	code := c.Query("code")
	hash := c.Query("hash")
	...
	//从oss获取文件
	fileData := lib.DownloadLocal(fileInfo.FileHash, fileInfo.Postfix)
	//下载次数+1
