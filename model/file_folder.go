...
func GetUserFileFolderCount(fileStoreId int) (fileFolderCount int) {
	...
	//.Find(&fileFolder, "file_store_id = ?", fileStoreId)‌
	//‌作用‌：查询符合条件的所有记录，并存储到 fileFolder 变量中。
	//‌参数‌：
	//&fileFolder：查询结果存储的目标变量（通常是结构体切片，如 []FileFolder）。
	//"file_store_id = ?"：SQL条件语句，? 是占位符，防止SQL注入。
	//fileStoreId：替换 ? 的实际参数值，表示要查询的 file_store_id。
